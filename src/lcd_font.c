 ...
 * https://github.com/geoffreymbrown/STM32-Template/blob/master/Library/glcdfont.c
 */
...
const lcd_font_character_t lcd_font_default[LCD_FONT_NO_CHARACTERS] = {
	{ 0x00, 0x00, 0x00, 0x00, 0x00 },  // 0
//	{ 0x3E, 0x5B, 0x4F, 0x5B, 0x3E },  // 1 - sad face
	{ 0x22, 0x14, 0x7F, 0x49, 0x36 },  // 1 - bluetooth icon
...
void lcd_font_print_char(int c, int line, int col, lcd_buffer_t buf) {
	for (int i = 0; i < LCD_FONT_DEFAULT_WIDTH; i++) {
		buf[line][col++] = lcd_font_default[c][i];
	}
	buf[line][col] = 0x00;
}

void lcd_font_print_string(const char* str, int line, int col, lcd_buffer_t buf) {
	while (*str) {
		lcd_font_print_char(*str, line, col, buf);
