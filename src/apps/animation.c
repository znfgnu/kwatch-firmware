...
static int active_elements = 0;
static unsigned int xx[APP__ANIMATION_MAX_ELEMENTS];
static unsigned int yy[APP__ANIMATION_MAX_ELEMENTS];
static unsigned int dx[APP__ANIMATION_MAX_ELEMENTS];
static unsigned int dy[APP__ANIMATION_MAX_ELEMENTS];

static void draw_handler(lcd_buffer_t buf) {
	for (int i = 0; i < active_elements; i++) {
		if (xx[i] + dx[i] >= LCD_WIDTH) dx[i] = -dx[i];
		if (yy[i] + dy[i] >= LCD_PAGES*8) dy[i] = -dy[i];	// negative values are GREATER than LCD_WIDTH, because x and y are unsigned
		xx[i] += dx[i];
		yy[i] += dy[i];
...
void app__animation_init() {
	...
	for (int i = 0; i < APP__ANIMATION_MAX_ELEMENTS; i++) {
		...
		dx[i] = pseudo_rand_get() % 7 - 3;	// -2 - 2
		dy[i] = pseudo_rand_get() % 7 - 3;	// -2 - 2
	}
